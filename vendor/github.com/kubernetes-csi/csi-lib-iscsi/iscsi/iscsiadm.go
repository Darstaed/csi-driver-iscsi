...
func iscsiCmd(args ...string) (string, error) {
	...
	klog.V(5).Infof("Run iscsiadm command: %s", strings.Join(append([]string{"iscsiadm"}, args...), " "))
...
func iscsiadmDebug(output string, cmdError error) {
	...
	klog.V(5).Infof("Output of iscsiadm command: {output: %s}", debugOutput)
	if cmdError != nil {
		klog.V(5).Infof("Error message returned from iscsiadm command: %s", cmdError.Error())
	}
}
...
func ListInterfaces() ([]string, error) {
	klog.V(5).Infof("Begin ListInterface...")
...
func ShowInterface(iface string) (string, error) {
	klog.V(5).Infof("Begin ShowInterface...")
...
func CreateDBEntry(tgtIQN, portal, iFace string, discoverySecrets, sessionSecrets Secrets) error {
	klog.V(5).Infof("Begin CreateDBEntry...")
	...
	if discoverySecrets.SecretsType == "chap" {
		klog.V(5).Infof("Setting CHAP Discovery...")
	...
	if sessionSecrets.SecretsType == "chap" {
		klog.V(5).Infof("Setting CHAP Session...")
...
func Discoverydb(tp, iface string, discoverySecrets Secrets, chapDiscovery bool) error {
	klog.V(5).Infof("Begin Discoverydb...")
...
func createCHAPEntries(baseArgs []string, secrets Secrets, discovery bool) error {
	args := []string{}
	klog.V(5).Infof("Begin createCHAPEntries (discovery=%t)...", discovery)
	if discovery {
...
func GetSessions() (string, error) {
	klog.V(5).Infof("Begin GetSessions...")
...
func DeleteDBEntry(tgtIQN string) error {
	klog.V(5).Infof("Begin DeleteDBEntry...")
...
func DeleteIFace(iface string) error {
	klog.V(5).Infof("Begin DeleteIFace...")
